...
Future<List<Marino>> ListMarino() async {
  final response =
      await http.get(Uri.parse("http://localhost:4000/api/marino"));
...
Future<List<Criollo>> listCriollo() async {
  final response =
      await http.get(Uri.parse("http://localhost:4000/api/criollo"));
...
Future<List<Parrilla>> listParrilla() async {
  final response =
      await http.get(Uri.parse("http://localhost:4000/api/parrilla"));
...
Future<List<Postre>> listPostres() async {
  final response =
      await http.get(Uri.parse("http://localhost:4000/api/postres"));
...
Future<List<Entradas>> listEntradas() async {
  final response =
      await http.get(Uri.parse("http://localhost:4000/api/entradas"));
...
Future<List<Tragos>> listTragos() async {
  final response =
      await http.get(Uri.parse("http://localhost:4000/api/tragos"));
